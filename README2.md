...
Você deve utilizar um `BrowserRouter` pra criar as rotas da sua aplicação. As urls de cada página deve ser conforme especificado na seção _O que será desenvolvido_. 

### 2 - Ao ser montado, `MovieList` deve fazer uma requisição para buscar a lista de filmes a ser renderizada
...
### 4 - Ao ser montado, `MovieDetails` deve fazer uma requisição para buscar o filme que deverá ser renderizado
...
### 4 - `MovieDetails` deve possuir um link para a página de edição de filmes.

No rodapé do cartão, deve haver um link com o texto "EDITAR" apontando para a rota `/movies/:id/edit` e um link apontando para a rota raiz (`/`) com o texto "VOLTAR", conforme ilustrado na imagem abaixo.

![image](card-details.png)

### 5 - Ao ser montado, `EditMovie` deve realizar uma requisição para buscar o filme que será editado.
...
### 6 - Na página inicial, deve haver um link para criar novos cartões.
...
### 6 - Ao submeter o formulário, `NewMovie` deve criar um novo filme e redirecionar o app para a página inicial.
